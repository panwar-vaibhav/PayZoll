...
export default function BalanceSection({ isWalletConnected }) {
  ...
  const totalBalance = cryptoBalances.reduce((acc, curr) => {
    return acc + parseFloat(curr.value.replace('$', '').replace(',', ''));
  }, 0);
  ...
  return (
    <div className="bg-crypto-card border border-gray-800 rounded-xl p-6 h-[420px] overflow-hidden hover:border-indigo-500/50 transition-all hover:shadow-lg hover:shadow-indigo-500/10 group">
      <div className="flex items-center justify-between mb-6">
        <div>
          <h2 className="text-2xl font-bold group-hover:bg-gradient-to-r group-hover:from-indigo-400 group-hover:to-purple-400 group-hover:bg-clip-text group-hover:text-transparent transition-all">Wallet Balance</h2>
          <p className="text-gray-400 text-sm mt-1">Total value in USD</p>
        </div>
        <button
          onClick={() => setShowAddFunds(true)}
          className="bg-gradient-to-r from-indigo-600 to-purple-600 text-white p-3 rounded-xl
                   hover:shadow-lg hover:shadow-indigo-500/20 transition-all flex items-center space-x-2 group-hover:scale-105"
        >
          ...
          >
            <div className="flex items-center justify-between">
              <div className="flex items-center space-x-3">
                ...
                <div>
                  <div className="font-semibold group-hover/item:text-white transition-colors">{crypto.currency}</div>
                  <div className="text-sm text-gray-400">{crypto.amount} {crypto.currency}</div>
                </div>
              </div>
              <div className="text-right">
                <div className="font-semibold group-hover/item:text-white transition-colors">{crypto.value}</div>
